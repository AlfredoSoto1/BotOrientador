		...
		event.getMember().getUser()
			.openPrivateChannel().queue(privateChannel -> {
				...
				privateChannel.sendMessage(String.format(
					...
					Yo soy el **Smart Assistant** del servidor y formo parte del equipo de %s, donde estamos organizados en sub-equipos para ayudarte mejor.
					
					Permíteme presentarte al equipo *%s*, uno de nuestros sub-equipos más destacados durante esta semana de orientación!. 
					...
					privateChannel.getUser().getEffectiveName(),
					server.getEmojisByName("Huella", true).get(0).getAsMention(),
					"ECE".equalsIgnoreCase(department) ? "TEAM-MADE" : "INSO/CIIC",
					"ALIENS"))
					.queue();
				
				privateChannel.sendMessageEmbeds(embed.buildWelcomeMessage(color)).queue();
